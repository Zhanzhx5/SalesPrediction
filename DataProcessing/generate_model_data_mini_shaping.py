...
print("🚀 开始Mini版数据处理（全部店铺+塑身商品）...")
print(f"开始时间: {datetime.now()}")

# 1. 读取基础数据
print("📖 读取基础数据...")
# 只读取塑身商品信息
goods_df = pd.read_csv('商品信息_塑身.csv', dtype=str, encoding='utf-8-sig', keep_default_na=False)
...
print(f"塑身商品信息: {len(goods_df)}行, 店铺信息: {len(store_df)}行")
...
all_items = goods_df['item_id'].unique()  # 只处理塑身商品

print(f"🎯 数据范围: 处理全部店铺，共{len(all_stores_full)}个")
print(f"🎯 商品范围: 仅塑身商品 {len(all_items)}个")
...
print(f"📊 预计总数据量: {len(all_dates) * len(all_stores) * len(all_items):,} 行 (Mini版)")
...
# 4. 读取销售数据并优化
print("💾 读取并优化销售数据...")
sales_df = pd.read_csv('销售信息_精简.csv', 
                      dtype={'dtdate': str, 'sstoreno': str, 'sgoodsskc': str, 'fquantity': str, '在店库存': str},
                      encoding='utf-8-sig', keep_default_na=False)

print(f"原始销售数据: {len(sales_df):,} 行")

# 数据清理（向量化）
sales_df = sales_df.rename(columns={'sstoreno': 'store_id', 'sgoodsskc': 'item_id'})
valid_mask = sales_df['dtdate'].str.match(r'^\d{4}-\d{2}-\d{2}$', na=False)
sales_df = sales_df[valid_mask]

sales_df['dtdate'] = pd.to_datetime(sales_df['dtdate'], errors='coerce')
sales_df['sales'] = pd.to_numeric(sales_df['fquantity'], errors='coerce').fillna(0)
sales_df['zd_kc'] = pd.to_numeric(sales_df['在店库存'], errors='coerce').fillna(0)

# 将负数的销量和在店库存改为0
sales_df['sales'] = sales_df['sales'].clip(lower=0)
sales_df['zd_kc'] = sales_df['zd_kc'].clip(lower=0)

sales_df = sales_df[(sales_df['dtdate'] >= start_date) & (sales_df['dtdate'] <= end_date)]
sales_df = sales_df.dropna(subset=['dtdate'])
...
# 🎯 只保留全部店铺的塑身商品销售数据
print(f"聚合后销售数据: {len(sales_df):,} 行")
sales_df_original_count = len(sales_df)
sales_df = sales_df[sales_df['store_id'].isin(all_stores) & sales_df['item_id'].isin(all_items)]
reduction_pct = (1 - len(sales_df) / sales_df_original_count) * 100
print(f"筛选全部店铺+塑身商品后销售数据: {len(sales_df):,} 行 (减少了 {reduction_pct:.1f}%)")
print(f"处理后销售数据: {len(sales_df):,} 行")
...
# 6. Mini版直接处理（不分批）
output_file = '../model_data_mini_shaping.csv'  # 输出到上级目录

print(f"🎯 输出文件: {output_file} (Mini版: 全部店铺+塑身商品)")

# 7. 创建基础框架
print("📊 创建基础框架...")
n_combinations = len(all_dates) * len(all_stores) * len(all_items)

# 创建基础DataFrame（内存优化）
chunk_df = pd.DataFrame({
    'dtdate': np.repeat(all_dates, len(all_stores) * len(all_items)),
    'store_id': np.tile(np.repeat(all_stores, len(all_items)), len(all_dates)),
    'item_id': np.tile(all_items, len(all_dates) * len(all_stores)),
    'sales': 0.0,
    'zd_kc': 0.0
})
...
if len(sales_df) > 0:
    # 使用merge操作（比iterrows快1000倍）
    chunk_df = chunk_df.merge(
        sales_df[['dtdate', 'store_id', 'item_id', 'sales', 'zd_kc']], 
...
def calculate_features_vectorized(group):
    ...
    # 2. 滚动统计特征（与LightGBM保持一致：基于前N天，不含当天）
    # 7天滚动（基于前7天，不含当天）
    ...
    # 3. 同比特征（与LightGBM保持一致：基于去年同期前7天）
...
# 13. 过滤保存数据（掐头去尾：只保留每个店铺-商品组合从第一次有销量到最后一次有销量的期间）
...
print(f"特征列数: {len(chunk_df_filtered.columns)}")
print(f"✅ 已保存 {len(chunk_df_filtered):,} 行 (掐头去尾处理)")
print(f"原始计算数据: {len(chunk_df):,} 行")
print(f"2023年后数据: {len(chunk_df_after_2023):,} 行")
print(f"掐头去尾后数据: {len(chunk_df_filtered):,} 行")

# 14. 输出统计
output_size = os.path.getsize(output_file) / (1024**3)

print("=" * 60)
print("🎉 Mini版数据处理完成! (全部店铺+塑身商品)")
print(f"结束时间: {datetime.now()}")
print(f"总计算行数: {len(chunk_df):,}")
print(f"最终保存行数: {len(chunk_df_filtered):,} (掐头去尾处理)")
...
print(f"- 💡 Mini版本，数据量大幅减少，适合快速测试和验证") 